...
class Producer:
    ...
    def publish(self, message: Any, properties: pika.BasicProperties = None, queue=None, callback_queue=None):
        ...
        try:
            ...
            logger.error(f'получил ошибку: {str(ex)}', )
            self.reconnect()
    ...
    def on_response(self, channel, method, properties, body: bytes):
        ...
        logger.info(f"Получил сообщение от клиента: {message}")
        ...
        if message_json['response'] == 4:
            sqlite_engine.update_status_device(message_json['device_id'], 'on')
            logger.info(f'данные теста устройства {message_json['device_id']}, {message_json['data']}')
        else:
